...
myApp.controller('TodoCtrl', function($scope) {
    ...
    $scope.ButtonClick = function() {
        ...
        while (nProcessos > 0) {
            ...
            $scope.processos.push({ nome, prioridade, duracao, start, processando, totalStatus: [] });
            nProcessos--;
        }
        ...
        function executaProcesso(indexProcesso) {
            ...
            quantum--;
            ultimoProcessoExecutado = $scope.processos[indexProcesso]['nome'];

            if (quantum == 0 && $scope.processos[indexProcesso]['duracao'] > 0) {
                $scope.processos[indexProcesso]['processando'] = false;
                processadorOcupado = false;
                filaCircular.unshift($scope.processos[indexProcesso]); //volta para a fila
                $scope.processos.splice(indexProcesso, 1);
                quantum = $scope.nQuantum;
            } else if ($scope.processos[indexProcesso]['duracao'] == 0) {
                verificaEncerrados();
                quantum = $scope.nQuantum;
                $scope.processos[indexProcesso]['processando'] = false;
                processadorOcupado = false;
        ...
        function verificaQuemComeca(instanteTempo) {
            ...
            try {
                ...
                //filaCircular = ordenaPorDuracao(filaCircular); //nao precisa
            ...
            $scope.processos = $scope.processos.filter(function(n) {
                return n != undefined });
        ...
        function ordenaProcessos() { //ordena por nome para exibiçao na tabela
            var k;
            var j;
            var copiaProcessos = new Array();
