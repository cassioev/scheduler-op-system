...
function executaProcesso(indexProcesso){
    ...
    $scope.processos[MaiorPrioridade]['naFila'] = true;
 ...
 function EncontraMaiorPrioridadeFila() {
        ...
        for (j = 0; j < $scope.processos.length; j++) {
        	if ($scope.processos[j]['naFila'] == true){
            	prioridadesFila.push($scope.processos[j]['prioridade']);
    	    }
    	}
        ...
        MaiorPrioridade = prioridadesFila[prioridadesFila.length-1];

        for (k = 0; k < $scope.processos.length; k++) {
            if ($scope.processos[k]['prioridade'] == MaiorPrioridade && $scope.processos[k]['naFila'] == true) {
